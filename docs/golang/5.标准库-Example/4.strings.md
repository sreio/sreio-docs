...
func strDemo() {
	...
	// 根据字符串创建reader  返回的是 reader.Reader
	...
	*/
	...
	// 按照字段排序 比较a和b, 返回 0: a等于b, 1: a大于b, -1: a小于b
	strings.Compare("a", "b") // -1

	// 判断a与b是否相同(将unicode大写、小写、标题三种格式字符视为相同)
	// utf-8字符串转为Unicode对比  所以不区分大小写
	...
	//fmt.Println(strings.TrimFunc("¡¡¡a-Hello1, 2Gophers-1!!!", func(r rune) bool {
	...
	strings.FieldsFunc("  foo1;bar2,，baz3...", func(c rune) bool {
		//fmt.Println(unicode.IsLetter(c), unicode.IsNumber(c), !unicode.IsLetter(c) && !unicode.IsNumber(c))
	...
	//fmt.Println(strings.ContainsAny("2saa伟大大啊", "伟"))

	// 判断是否包含rune字符
	strings.ContainsRune("I like seafood.", 'f')
	//fmt.Println(strings.ContainsRune("aar伟大大", 'a'))
