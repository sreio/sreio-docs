---
title: 结构体
createTime: 2025/07/04 15:08:42
permalink: /golang/golang_study/结构体/
---

::: tip Go语言中没有“类”的概念，也不支持“类”的继承等面向对象的概念。Go语言中通过`结构体`的内嵌再配合接口比面向对象具有更高的扩展性和灵活性。
:::

## 自定义类型/类型别名

::: note 自定义类型语法： `type MyInt int` //将MyInt定义为int类型
...
::: note Go语言中的方法（Method）是一种作用于特定类型变量的函数。这种特定类型变量叫做接收者（Receiver）。接收者的概念就类似于其他语言中的this或者 self。
:::
...
::: tip 结构体中字段大写开头表示可公开访问，小写表示私有（仅在定义当前结构体的包中可访问）。
:::
