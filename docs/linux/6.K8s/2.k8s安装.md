---
title: k8s安装
createTime: 2025/07/07 14:40:47
permalink: /linux/k8s安装/
---
...
::: note 然后通过 `xshell` 等工具连接这三台服务器的root用户
:::
...
::: note 关闭 `Linux` 的 `swap` 分区，提升 `Kubernetes` 的性能。
:::
...
::: note `Kubernetes` 使用主机名来区分集群里的节点，所以每个节点的 `hostname` 必须不能重名。
:::
...
::: note 修改完 `hostname` 后，重新登录，当前命令行界面连接 `hostname` 才会更新。
:::
...
::: note Ubuntu有三个版本，分别是bionic、focal、jammy
...
::: note 也可以锁定这三个软件的版本，避免意外升级导致版本错误
:::
...
::: tip ***重点***: `calico`  和 `flannel` 二选一即可， 前者功能更加完善且复杂
:::

::: note ***镜像替换地址***：https://docker.aityp.com/
:::
