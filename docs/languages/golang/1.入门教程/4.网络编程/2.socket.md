...
permalink: /golang/golang_study/socket/
---
::: tip `Socket`是`BSD UNIX`的进程通信机制，通常也称作”`套接字`”，用于描述`IP地址`和`端口`，是一个通信链的句柄。`Socket`可以理解为`TCP/IP`网络的API，它定义了许多函数或例程，程序员可以用它们来开发`TCP/IP`网络上的应用程序。电脑上运行的应用程序通常通过”`套接字`”向网络发出请求或者应答网络请求。
::: 
## socket图解
